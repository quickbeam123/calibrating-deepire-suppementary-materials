...
SATURATION_ALGORITHM = "lrs --decode lrs+1002_3:2_aac=none:add=on:bce=on:cond=fast:eape=off:er=filter:erml=3:fdtod=off:ins=6:kws=arity_squared:lma=off:lrd=on:lwlo=on:nm=32:s2agt=16:s2at=1.5:s2pl=no:sfv=off:sp=const_max:spb=goal:tgt=ground:to=kbo_0" # can also be "discount" or "otter" (lrs needs special treatment, to save traces for reproducibility)

PROBLEM_LIST = "mizar_shuf_head15000.txt" # newly, we don't want to run on SAT
NUM_TRAIN_PROBLEMS = 15000 # 15500 is the full set
EVAL_ON_TEST = False
